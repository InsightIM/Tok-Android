package com.client.tok.media.recorder;
...
public class OpusAudioRecorder {
    ...
    public void stopRecording(boolean send, boolean vibrate) {
    ...
    private void stopRecordingInternal(boolean send) {
        ...
        fileEncodingQueue.postRunnable(() -> {
            ...
            if (send) {
                ...
                byte[] waveForm = getWaveform2(recordSamples, recordSamples.length);
    ...
    private class RecordRunnable implements Runnable {
        ...
        public void run() {
            if (audioRecord != null) {
                short[] shortArray = new short[recordBufferSize];
                int len = audioRecord.read(shortArray, 0, shortArray.length);
                if (len > 0 && !callStop) {
                    int sum = 0;
                    try {
