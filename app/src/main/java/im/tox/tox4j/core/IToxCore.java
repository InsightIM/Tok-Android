 ...
 *
 * This interface is designed to be thread-safe. However, once [[ToxCore.saveAndClose]] has been called, all subsequent calls
 * will result in [[im.tox.tox4j.exceptions.ToxKilledException]] being thrown. When one thread invokes
 * [[ToxCore.saveAndClose]], all other threads with pending calls will throw. The exception is unchecked, as it should not occur
 * in a normal execution flow. To prevent it from occurring in a multi-threaded environment, all additional threads
 * should be stopped or stop using the instance before one thread invokes [[ToxCore.saveAndClose]] on it, or appropriate
...
public interface IToxCore extends ToxCoreEventSynth {
    ...
    IToxCore load(ToxOptions options) throws ToxNewException;
    ...
    void setSignature(ToxStatusMessage message) throws ToxSetInfoException;
     ...
     * This is the byte array the friend got from their own [[getSelfAddress]].
     * @param message the message to send with the friend request (must not be empty).
     * @return the new friend's friend number.
     */

    ToxFriendNumber sendAddFriendRequest(ToxFriendAddress address, ToxFriendRequestMessage message)
     ...
     * @return the new friend's friend number.
     */
    ToxFriendNumber acceptAddFriendRequest(ToxPublicKey publicKey)
        throws ToxFriendAddException, IllegalArgumentException;
     ...
     *
     * When a friend goes offline, all file transferManager associated with the friend are
     * purged from core.
     *
     ...
     * uniquely identify file transferManager across core restarts. If empty, a random one will
